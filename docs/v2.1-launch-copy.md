...
Save them as `AUTH_TOKEN` and `CT0` in `~/.config/peter-wanna-know/.env` or `.claude/peter-wanna-know.env`:
```bash
...
**Verify it's working:**
```bash
node ~/.claude/skills/peter-wanna-know/scripts/lib/vendor/bird-search/bird-search.mjs --whoami
```
...
/peter-wanna-know v2.1 just shipped — two headline features:
