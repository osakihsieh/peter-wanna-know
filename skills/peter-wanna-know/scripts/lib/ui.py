"""Terminal UI utilities for peter-wanna-know skill."""
...
MINI_BANNER = f"""{Colors.PURPLE}{Colors.BOLD}/peter-wanna-know{Colors.RESET} {Colors.DIM}· researching...{Colors.RESET}"""
...
To fix this:
1. Add AUTH_TOKEN and CT0 to ~/.config/peter-wanna-know/.env or .claude/peter-wanna-know.env
...
class ProgressDisplay:
    ...
    def _show_banner(self):
        if IS_TTY:
            ...
            sys.stderr.write(f"/peter-wanna-know · researching: {self.topic}\n")
        sys.stderr.flush()
...
def show_diagnostic_banner(diag: dict):
    ...
    if IS_TTY:
        ...
        _header = f"/peter-wanna-know v{_skill_version()} - Source Status"
        ...
        lines.append(f"{Colors.DIM}│{Colors.RESET}                                                     {Colors.DIM}│{Colors.RESET}")
        lines.append(f"{Colors.DIM}│{Colors.RESET}  Config: {Colors.BOLD}~/.config/peter-wanna-know/.env{Colors.RESET}                  {Colors.DIM}│{Colors.RESET}")
        lines.append(f"{Colors.DIM}└─────────────────────────────────────────────────────┘{Colors.RESET}")
    else:
        # Plain text for non-TTY (Claude Code / Codex)
        lines.append("┌─────────────────────────────────────────────────────┐")
        _header_plain = f"/peter-wanna-know v{_skill_version()} - Source Status"
        ...
        lines.append("│                                                     │")
        lines.append("│  Config: ~/.config/peter-wanna-know/.env                  │")
        lines.append("└─────────────────────────────────────────────────────┘")
