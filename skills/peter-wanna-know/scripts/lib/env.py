"""Environment and API key management for peter-wanna-know skill."""
...
if _config_override == "":
    ...
    CONFIG_DIR = Path.home() / ".config" / "peter-wanna-know"
    CONFIG_FILE = CONFIG_DIR / ".env"
...
# Example: `security add-generic-password -a "$USER" -s peter-wanna-know-XAI_API_KEY -w "xai-..."`.
KEYCHAIN_SERVICE_PREFIX = "peter-wanna-know-"
...
def _check_file_permissions(path: Path) -> None:
    ...
    try:
        ...
        if mode & 0o044:
            sys.stderr.write(
                f"[peter-wanna-know] WARNING: {path} is readable by other users. "
                f"Run: chmod 600 {path}\n"
            )
            sys.stderr.flush()
    except OSError as exc:
        sys.stderr.write(f"[peter-wanna-know] WARNING: could not stat {path}: {exc}\n")
        sys.stderr.flush()
...
def _decode_jwt_payload(token: str) -> dict[str, Any] | None:
    ...
    try:
        ...
        sys.stderr.write(f"[peter-wanna-know] WARNING: malformed JWT token: {exc}\n")
        sys.stderr.flush()
        return None
...
def load_codex_auth(path: Path = CODEX_AUTH_FILE) -> dict[str, Any]:
    ...
    try:
        ...
        sys.stderr.write(
            f"[peter-wanna-know] WARNING: {path} exists but contains invalid JSON -- ignoring\n"
        )
        sys.stderr.flush()
        return {}
...
def _find_project_env() -> Path | None:
    ...
    Searches for .claude/peter-wanna-know.env in each parent directory,
    stopping at the user's home directory or filesystem root.
    """
    cwd = Path.cwd()
    for parent in [cwd, *cwd.parents]:
        candidate = parent / '.claude' / 'peter-wanna-know.env'
        if candidate.exists():
            return candidate
...
def get_config() -> dict[str, Any]:
    ...
    Priority (highest wins):
      ...
      2. .claude/peter-wanna-know.env (per-project config)
      3. ~/.config/peter-wanna-know/.env (global config)
      4. macOS Keychain items prefixed ``peter-wanna-know-`` (Darwin only)
    """
...
def is_xiaohongshu_available(config: dict[str, Any]) -> bool:
    ...
    try:
        ...
        sys.stderr.write(
            f"[peter-wanna-know] WARNING: unexpected error checking Xiaohongshu: "
