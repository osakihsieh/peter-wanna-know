...
def ensure_supported_python(version_info: tuple[int, int, int] | object | None = None) -> None:
    ...
    if (major, minor) >= MIN_PYTHON:
        return
    sys.stderr.write(
        "peter-wanna-know v3 requires Python 3.12+.\n"
...
def slugify(value: str) -> str:
    ...
    return slug or "peter-wanna-know"
...
def read_synthesis_file(path: str) -> str:
    try:
        ...
        sys.stderr.write(f"[peter-wanna-know] Cannot read --synthesis-file: {exc}\n")
        raise SystemExit(2)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default=None,
        metavar="N",
        help="Auto-discover N competitor entities and fan out peter-wanna-know across all of them as a comparison (default N=2 → 3-way: original + 2 peers; range 1..6). Use --competitors-list to override discovery.",
    )
    parser.add_argument(
        "--competitors-list",
...
def main() -> int:
    ...
    if args.synthesis_file:
        if args.emit == "html":
            ...
            sys.stderr.write("[peter-wanna-know] Warning: --synthesis-file is only used with --emit=html; ignoring.\n")
            ...
            if comp_explicit:
                ...
                if not resolve_mod._has_backend(config) and not args.mock:
                    sys.stderr.write(
                        ...
                        "  3. Re-invoke: /peter-wanna-know '{topic} vs {peer1} vs {peer2}' "
    ...
    # ~/.config/peter-wanna-know/.env, which env.py loads but does not propagate
    ...
    if args.store or _store_env in ("1", "true", "yes"):
        ...
        sys.stderr.write(
            f"[peter-wanna-know] Stored {counts['new']} new, {counts['updated']} updated findings\n"
        )
        sys.stderr.flush()
    ...
    if args.save_dir:
        ...
        sys.stderr.write(f"[peter-wanna-know] Saved output to {save_path}\n")
        ...
        if entity_reports and len(entity_reports) > 1:
            for label, entity_report in entity_reports[1:]:
                ...
                sys.stderr.write(f"[peter-wanna-know] Saved output to {peer_path}\n")
        sys.stderr.flush()
    print(rendered)
    return 0
