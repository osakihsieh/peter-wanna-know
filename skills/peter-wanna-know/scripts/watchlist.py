#!/usr/bin/env python3
"""Topic watchlist management for peter-wanna-know."""
...
def _format_delivery_message(topic: str, counts: dict, mode: str) -> str:
    ...
    if mode == "announce":
        return f"📰 *peter-wanna-know update: {topic}*\n{new} new, {updated} updated"
    elif mode == "silent":
        return f"peter-wanna-know: {new} new findings for '{topic}'"
    else:
        return f"peter-wanna-know: Research complete for '{topic}'"
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Manage the peter-wanna-know watchlist")
