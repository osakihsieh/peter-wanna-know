...
# Usage: bash skills/peter-wanna-know/scripts/compare.sh "Kanye West"
#
# Runs /peter-wanna-know (public release) and /peter-wanna-know-beta (private beta)
...
if [ $# -eq 0 ]; then
  echo "Usage: bash skills/peter-wanna-know/scripts/compare.sh <topic>"
  echo "  Example: bash skills/peter-wanna-know/scripts/compare.sh Kevin Rose"
  exit 1
fi
TOPIC="$*"
...
echo "[1/2] Running /peter-wanna-know (public release)..."
echo "  This takes 2-4 minutes..."
claude -p --dangerously-skip-permissions "/peter-wanna-know $TOPIC" > /dev/null 2>&1 || true
...
# Run 2: private beta
echo "[2/2] Running /peter-wanna-know-beta (private beta)..."
echo "  This takes 2-4 minutes..."
claude -p --dangerously-skip-permissions "/peter-wanna-know-beta $TOPIC" > /dev/null 2>&1 || true
...
echo "  🧪 peter-wanna-know-beta · branch <name> · synced $DATE"
