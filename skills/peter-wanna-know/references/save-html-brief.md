...
SYNTHESIS_FILE="/tmp/peter-wanna-know-synthesis-${CLAUDE_SESSION_ID}.md"
...
"${LAST30DAYS_PYTHON}" "${SKILL_ROOT}/scripts/peter-wanna-know.py" "${TOPIC}" \
  --emit=html \
...
- The badge (`🌐 peter-wanna-know vX.Y.Z · synced YYYY-MM-DD`) at the top
...
If the user runs `/peter-wanna-know OpenClaw` normally, sees the synthesis in chat, and THEN says "save that as HTML" or "give me a shareable version" in a follow-up turn, do the same save flow on the synthesis you wrote in the previous turn. Do not re-research; the synthesis is already in the conversation history. Just write it to the temp file and call the engine with `--emit=html --synthesis-file`.
...
- **No `${LAST30DAYS_MEMORY_DIR}` set**: defaults to `~/Documents/peter-wanna-know/` per the SKILL.md `Configuration` section.
