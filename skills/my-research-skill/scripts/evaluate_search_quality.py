#!/usr/bin/env python3
"""Compare two peter-wanna-know revisions on the v3 ranked candidate output."""
...
def run_peter-wanna-know(repo_dir: Path, topic: str, *, search: str, timeout_seconds: int, quick: bool, mock: bool, env: dict[str, str]) -> dict[str, Any]:
    engine = repo_dir / "skills" / "peter-wanna-know" / "scripts" / "peter-wanna-know.py"
...
def create_worktree(rev: str) -> Path:
    worktree_dir = Path(tempfile.mkdtemp(prefix="peter-wanna-know-eval-"))
    subprocess.run(
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Compare two peter-wanna-know revisions on ranked candidate quality")
...
def main() -> int:
    ...
    try:
        ...
        for topic, query_type in topics:
            try:
                baseline_report = run_peter-wanna-know(
                    baseline_dir,
                    topic,
                    search=args.search,
                ...
                candidate_report = run_peter-wanna-know(
                    candidate_dir,
                    topic,
                    search=args.search,
                    timeout_seconds=args.timeout,
