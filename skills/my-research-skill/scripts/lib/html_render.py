"""HTML rendering for shareable peter-wanna-know reports."""
...
CANONICAL_BOUNDARY_PATTERN = re.compile(r"\n?---\n# END OF peter-wanna-know CANONICAL OUTPUT.*$", re.DOTALL)
...
<title>peter-wanna-know · __TITLE__</title>
...
def _markdown_to_html(md: str) -> str:
    ...
    while index < len(lines):
        ...
        if stripped.startswith("🌐 peter-wanna-know"):
            flush_paragraph()
            close_list()
...
def _build_colophon(report: schema.Report, *, topic: str | None = None) -> str:
    ...
    rerun = html.escape(f"/peter-wanna-know {display_topic}")
    return (
        '<div class="colophon">\n'
        f"  Generated {generated} by /peter-wanna-know v{html.escape(version)} · topic: {escaped_topic}<br>\n"
