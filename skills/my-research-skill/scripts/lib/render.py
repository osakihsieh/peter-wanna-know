...
def _render_badge() -> list[str]:
    ...
    return [
        f"🌐 peter-wanna-know v{version} · synced {today}",
        "",
    ]
...
def render_compact(report: schema.Report, cluster_limit: int = 8, fun_level: str = "medium", save_path: str | None = None) -> str:
    ...
    lines = [
        *_render_badge(),
        f"# peter-wanna-know v{_skill_version()}: {report.topic}",
        "",
...
def _render_canonical_boundary() -> list[str]:
    ...
    return [
        "",
        "---",
        "# END OF peter-wanna-know CANONICAL OUTPUT",
        "",
        ...
        "apply to peter-wanna-know output.",
    ]
...
def _render_pre_research_warning(report: schema.Report) -> list[str]:
    ...
    return [
        ...
        f"then rerun `/peter-wanna-know {report.topic}`. The skill will resolve handles",
...
def _render_degraded_run_warning(report: schema.Report) -> list[str]:
    ...
    return [
        ...
        "Hermes, Gemini, or any agent runtime that invoked `/peter-wanna-know`): YOU ARE",
...
def render_comparison_multi(
    ...
    *,
    ...
    lines: list[str] = [
        *_render_badge(),
        f"# peter-wanna-know v{_skill_version()}: {synthesized_topic}",
        "",
        *_assistant_safety_lines(),
